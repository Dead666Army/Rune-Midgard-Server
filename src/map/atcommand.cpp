...
ACMD_FUNC(go)
{
	...
	const struct {
		...
		{ MAP_YUNO,        157, 124 }, //  9=Yuno
		{ MAP_AMATSU,      108, 150 }, // 10=Amatsu
		{ MAP_GONRYUN,     160, 120 }, // 11=Gonryun
		{ MAP_UMBALA,       89, 157 }, // 12=Umbala
		{ MAP_NIFLHEIM,    195, 186 }, // 13=Niflheim
		...
		{ MAP_JAWAII,      215, 164 }, // 17=Jawaii
		{ MAP_AYOTHAYA,    218, 174 }, // 18=Ayothaya
		{ MAP_EINBROCH,    233, 202 }, // 19=Einbroch
		{ MAP_LIGHTHALZEN, 158,  92 }, // 20=Lighthalzen
		{ MAP_EINBECH,     170, 131 }, // 21=Einbech
...
ACMD_FUNC(credits) {
	...
	char output[100];
	char *data;
	if (SQL_ERROR == Sql_Query(mmysql_handle, "select `value` from `acc_reg_num` WHERE `account_id` = %d and `key` = '%s' ;", sd->status.account_id, "#credits")) {
		Sql_ShowDebug(mmysql_handle);
		return 1;
	}
	...
	//credits = 5 - pc_readaccountreg(sd, add_str("#credits"));
	sprintf(output, msg_txt(sd, 1505), credits); // You have %d credits.
	clif_displaymessage(fd, output);
	return 0;
}

ACMD_FUNC(creditsparty) {
	int credits;
	int party_id;
	struct party_data *p;
	char *data;
	...
	if (party_id != 0) {
		if ((p = party_search(party_id)) != NULL) {
			party_member_data *pd = p->data;
			for (int i = 0; i < MAX_PARTY; i++) {
				if (pd->sd != NULL) {
					if (SQL_ERROR == Sql_Query(mmysql_handle, "select `value` from `acc_reg_num` WHERE `account_id` = %d and `key` = '%s' ;", pd->sd->status.account_id, "#credits")) {
						Sql_ShowDebug(mmysql_handle);
						return 1;
					}
					if (SQL_SUCCESS != Sql_NextRow(mmysql_handle))
					...
					Sql_FreeResult(mmysql_handle);
					//credits = 5 - pc_readaccountreg(pd->sd, add_str("#credits"));
					sprintf(output, msg_txt(sd, 1506), pd->sd->status.name, credits); // You have %d credits.
					clif_displaymessage(fd, output);
		...
		clif_displaymessage(fd, msg_txt(sd, 1507));// You do not have a team.
		return -1;
	}
	return 0;
}
...
ACMD_FUNC(refresh)
{
	...
	if (DIFF_TICK(gettick(), sd->refresh_cd) < 1000) {
		char timeleft[100];
		sprintf(timeleft, "@refresh is cooling down, you may use it again after %d seconds", 1 - (DIFF_TICK(gettick(), sd->refresh_cd) / 1000));
		clif_displaymessage(fd, timeleft);
		return 0;
	}
	sd->refresh_cd = gettick();
	clif_refresh(sd);
	return 0;
}
...
ACMD_FUNC(whobuy)
{
	...
	if ((item_data = itemdb_searchname(item_name)) == NULL &&
		(item_data = itemdb_exists(atoi(item_name))) == NULL)
	{
		clif_displaymessage(fd, "Item not found"); // Invalid item ID or name.
		return -1;
	}
