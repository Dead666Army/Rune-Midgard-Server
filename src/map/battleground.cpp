...
void bg_block_skill_start(struct battleground_data *bgd, int skillnum, unsigned int time)
{
...
void bg_team_rewards(int bg_id, int nameid, int amount, int kafrapoints, int quest_id, const char *var, int add_value, int bg_arena, int bg_result)
{
	...
	for (j = 0; j < MAX_BG_MEMBERS; j++)
	{
		...
		switch( bg_result ) {
			case 0: // Won
				//Add Exp for each win
				if (battle_config.bg_base_exp || battle_config.bg_job_exp)
					pc_gainexp(sd, NULL, battle_config.bg_base_exp*2, battle_config.bg_job_exp*2, 1);

				break;
			case 1: // Tie
				//Add Exp for each win
				if (battle_config.bg_base_exp || battle_config.bg_job_exp)
					pc_gainexp(sd, NULL, battle_config.bg_base_exp * 1.5, battle_config.bg_job_exp * 1.5, 1);

				break;
			case 2: // Lost
				//Add Exp for each win
				if (battle_config.bg_base_exp || battle_config.bg_job_exp)
					pc_gainexp(sd, NULL, battle_config.bg_base_exp * 1, battle_config.bg_job_exp * 1, 1);
