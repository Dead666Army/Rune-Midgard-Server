...
/**
 * Give Base or Job percent of current level to player
 * @param sd Player
 * @param src EXP source
 * @param base_exp_percent Base EXP gained
 * @param job_exp_percent Job EXP gained
 * @param exp_flag 1: Quest EXP; 2: Param Exp (Ignore Guild EXP tax, EXP adjustments)
 * @return
 **/
...
int pc_dead(struct map_session_data *sd,struct block_list *src)
{
	...
	if (src && src->type == BL_PC) {
		...
		//woe stats
		if (mapdata->flag[MF_GVG_CASTLE] && is_agit_start()) {
			if (sd && sd->status.char_id) {
				add2limit(sd->status.woestats.death_count, 1, USHRT_MAX);
			}
			if (ssd->status.char_id && ssd->status.char_id != sd->status.char_id) {
				add2limit(ssd->status.woestats.kill_count, 1, USHRT_MAX);
			}
		}
	}
...
int pc_itemheal(struct map_session_data *sd, int itemid, int hp, int sp)
{
	...
	if (hp) {
		...
		//woe stats
		if (map_getmapflag(sd->bl.m, MF_GVG_CASTLE) && sd->status.char_id && is_agit_start())
			add2limit(sd->status.woestats.hp_heal_potions, 1, UINT_MAX);
	}
	if (sp) {
		...
		//woe stats
		if (map_getmapflag(sd->bl.m, MF_GVG_CASTLE) && sd->status.char_id && is_agit_start())
			add2limit(sd->status.woestats.sp_heal_potions, 1, UINT_MAX);
	}
	if (sd->sc.count) {
