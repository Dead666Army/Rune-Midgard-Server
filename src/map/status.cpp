...
int status_damage(struct block_list *src,struct block_list *target,int64 dhp, int64 dsp, int walkdelay, int flag)
{
	...
	//Woe stats
	if (src && src->type == BL_PC && map_getmapflag(src->m, MF_GVG_CASTLE) && is_agit_start()) {
		TBL_PC *ssd = BL_CAST(BL_PC, src);
		if (ssd->status.char_id) {
			if (target->type == BL_PC) {
				TBL_PC *tssd = BL_CAST(BL_PC, target);
				add2limit(ssd->status.woestats.damage_done, hp, UINT_MAX);
				add2limit(tssd->status.woestats.damage_received, hp, UINT_MAX);
				if (ssd->status.woestats.top_damage < hp)
					ssd->status.woestats.top_damage = hp;
			}
			else if (target->type == BL_MOB) { // emp dmg
				struct mob_data *md = (struct mob_data *)target;
				if (md->mob_id == MOBID_EMPERIUM) {
					add2limit(ssd->status.woestats.emp_damage_done, hp, UINT_MAX);
					if (status->hp <= 0) {
						add2limit(ssd->status.woestats.emperium_kill, 1, UINT_MAX);
					}
				}
			}
		}
...
//		if (status_get_race2(bl) == RC2_GVG && status_sc2scb_flag(type)&SCB_MAXHP) return 0;
	...
	switch(type)
	{
      ...
      status_set_hp(bl, 99999, 0); //FULL HEAL LIF
			status_set_sp(bl, 99999, 0); //FULL HEAL LIF
			break;
		case SC_SWOO:
